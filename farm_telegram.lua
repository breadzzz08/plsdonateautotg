...
function sendTelegramMessage(text)
    local token = "8102915980:AAH1lHQnRoW_c3jN9PLNK5L5jBwlnJpTDAg"
    local chat_id = "1465358774"
...
local function sendTelegramMessage(raised, donor)
...
if CoreGui:FindFirstChild('RobloxPromptGui') then
	for i, v in next, CoreGui.RobloxPromptGui:GetDescendants() do
		if v:IsA('TextLabel') and string.find(string.lower(v.Text), "ban") then
			sendTelegramMessage('@everyone ||' .. Players.LocalPlayer.Name .. '|| got banned' .. v.Text:gsub("You've been banned", ''):gsub('You were kicked from this experience:', ''):gsub('(Error Code: 267)', ''):gsub('()', ''))
		end
	end
end
...
webhookTab:AddButton("Test Webhook", function()
	if getgenv().settings.webhookBox:gsub(" ","") ~= "" then
		sendTelegramMessage("Webhook works!")
	end
end)
		...
		task.spawn(function()
			...
			if playerWhoDonated then
				if getgenv().settings.webhookType == 'New' then
					pcall(function()
						sendTelegramMessage(raised, playerWhoDonated.Name)
					end)
				else
					pcall(function()
						sendTelegramMessage(Players.LocalPlayer.Name .. ' | Donation amount: ' .. tostring(raised) .. ' | [A/T]: ' .. tostring(math.floor((raised) * 0.6)) .. ' | Total: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value) .. ' | Donor: ' .. playerWhoDonated.Name)
					end)
				end
			else
				if getgenv().settings.webhookType == 'New' then
					pcall(function()
						sendTelegramMessage(raised, "Hi, I'm Crazyblox.")
					end)
				else
					pcall(function()
						sendTelegramMessage(Players.LocalPlayer.Name .. ' | Donation amount: ' .. tostring(raised) .. ' | [A/T]: ' .. tostring(math.floor((raised) * 0.6)) .. ' | Total: ' .. tostring(Players.LocalPlayer.leaderstats.Raised.Value))
					end)
				end
			end
		end)
...
if getgenv().settings.webhookAfterSH then
	if Players.LocalPlayer.DisplayName ~= Players.LocalPlayer.Name then
		sendTelegramMessage(Players.LocalPlayer.DisplayName .. ' (@' .. Players.LocalPlayer.Name .. ') serverhopped')
	else
		sendTelegramMessage('@' .. Players.LocalPlayer.Name .. ' serverhopped')
	end
end
